...
class Button:
    ...
    def tick(self):
        ...
        self.game_ref.screen.blit(image,[self.x * 100*self.game_ref.qsc, self.y * 100*self.game_ref.qsc + smoothing])
        pygame.draw.rect(self.game_ref.screen, color, [self.x * 100*self.game_ref.qsc, self.y * 100*self.game_ref.qsc + smoothing, self.button_sizex*self.game_ref.qsc, self.button_sizey*self.game_ref.qsc],width)
        if self.key_press != "":
            core.func.render_text(self.game_ref, self.key_press.upper(), [self.x * 100*self.game_ref.qsc+5, self.y * 100*self.game_ref.qsc + smoothing+5],20, color = color)
